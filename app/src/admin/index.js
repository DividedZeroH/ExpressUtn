...
const safeDelete = async (request, response, context) => {
  ...
  try {
    ...
    const isFk =
      ...
      /foreign key|violates foreign key/i.test(err.message || '');
    const msg = isFk
      ? 'No se puede eliminar: el registro está siendo usado por una venta (clave foránea).'
      : `Error al eliminar: ${err.message}`;
    // IMPORTANTE: redirigir también en error. Sin redirectUrl, la acción
    // delete (component: false) deja el SPA en pantalla negra.
    return {
      record: record.toJSON(currentAdmin),
      redirectUrl: listUrl,
    ...
    },
    {
      ...
      options: {
        ...
        properties: {
          ...
          bebida_id: { isRequired: true, reference: 'Bebida' },
          barra_id:  { isRequired: true, reference: 'Barra' },
